...
@EventBusSubscriber(modid = Verdure.MOD_ID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public class ClientEventHandler {
    ...
    public static void registerBlockColors(RegisterColorHandlersEvent.Block event) {
        event.register(
            (state, level, pos, tintIndex) -> level != null && pos != null ? BiomeColors.getAverageFoliageColor(
                level,
                pos
            ) : FoliageColor.getDefaultColor(),
            VerdureBlocks.FALLEN_OAK_LEAVES.get(),
            VerdureBlocks.FALLEN_JUNGLE_LEAVES.get(),
            VerdureBlocks.FALLEN_ACACIA_LEAVES.get(),
            VerdureBlocks.FALLEN_DARK_OAK_LEAVES.get(),
            VerdureBlocks.OAK_BRANCH.get()
        );
        event.register(
            (pState, pLevel, pPos, pTintIndex) -> FoliageColor.getEvergreenColor(),
            VerdureBlocks.FALLEN_SPRUCE_LEAVES.get()
        );
        event.register(
            (pState, pLevel, pPos, pTintIndex) -> FoliageColor.getBirchColor(),
            VerdureBlocks.FALLEN_BIRCH_LEAVES.get(),
            VerdureBlocks.BIRCH_BRANCH.get()
        );
    }
